...
class UserProfile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<AppCubit, AppStates>(
      ...
      builder: (context, state) {
        ...
        UserModel user = findUserById(uId, cubit);

        return Scaffold(
          appBar: _buildAppBar(context, cubit, user),
          body: _buildBody(context, cubit, user),
        );
      },
    );
  }

  AppBar _buildAppBar(BuildContext context, AppCubit cubit, UserModel user) {
    return AppBar(
      title: Text(
        "${user.name} Profile",
        maxLines: 1,
  ...
  Widget _buildBody(BuildContext context, AppCubit cubit, UserModel user) {
    return ListView.builder(
      itemCount: user.uPosts.length + 1,
      itemBuilder: (context, index) {
        if (index == 0) {
          return _buildUser(user, cubit, context);
        } else {
          String postId = user.uPosts[index - 1];
  ...
  Widget _buildUser(UserModel user, AppCubit cubit, BuildContext context) {
    return Column(
        ...
        Padding(
          ...
          child: Column(
            children: [
              ...
              SizedBox(height: 10),
              Text(user.phone, style: AppTheme().bodySmall),
              SizedBox(height: 20),
              myDivider(),
              Row(
                children: [
              ...
              buildDefaultButton(
                () {
                  if (user.isEmailVerified == false) {
                    ...
                    navigateToBack(context, UserChat(rId: user.uId));
                  }
                },
                'Message',
